...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        try {
            ...
            // дублюємо власний cookie, якщо він використовується (boot_session робить схоже)
            if (!headers_sent()) {
                setcookie('dfbiu_sess', session_id(), [
                    'path' => '/',
                    'httponly' => true,
                    'samesite' => 'Lax'
                ]);
...
<style>
  ...
  .input,.select{width:100%;height:50px;padding:0 14px 0 44px;border:1px solid var(--border);border-radius:12px;
...
<div class="reg-root">
  <form class="card" action="" method="post" autocomplete="off">
    ...
    <div class="links">已有账号？ <a href="/dfbiu/login">立即登录</a></div>
  </form>
</div>
</body>
</html>
